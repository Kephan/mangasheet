<?php
    ...
    $sql = "SELECT * FROM comentarios a WHERE $coleccion = a.colID AND a.replyID != 0 AND a.volnum = $volumen ORDER BY fecha ASC"; 
    
    $result = mysqli_query($link, $sql) or die(mysqli_error($link));  
    while($row = mysqli_fetch_array($result)){?>    
    <div class="comment-box reply-comment-box" data-reply-id="<?php echo $row['replyID'] ?>" data-id="<?php echo $row['id'] ?>">
